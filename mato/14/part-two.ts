...
const normalizeVectors = (vectors: Vector[]) => {
  ...
  const maxY =
    vectors.reduce((max, vector) => Math.max(max, vector.y1, vector.y2), 0) + 2;

  vectors.push(new Vector([500 - maxY, maxY], [500 + maxY, maxY]));

  const minX = vectors.reduce(
    (min, vector) => Math.min(min, vector.x1, vector.x2),
    500,
  );
...
const addOneSand = (grid: GridElement[][], minX: number) => {
  ...
  if (grid[sandY][sandX] === GridElement.Sand) {
    // Pile has reached the top
    return false;
  }
