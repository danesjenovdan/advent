...
export default (lines: string[]) => {
  ...
  const doubleScores = allPaths.map((path, index) => {
    const score = scores[index];
    let bestAdditional = 0;
    allPaths.forEach((elephantPath) => {
      elephantPath = elephantPath.filter((item) => !path.includes(item));
      elephantPath.unshift("AA");
      const additionalScore = ratePath(elephantPath, graph);
      bestAdditional = Math.max(bestAdditional, additionalScore);
    });

    return score + bestAdditional;
  });

  return Math.max(...doubleScores);
};
